-- Migración corregida para crear tabla de clientes
...
-- Agregar columna client_uuid a la tabla consents si no existe
-- Usamos client_uuid en lugar de client_id porque client_id ya existe como VARCHAR
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE consents ADD COLUMN client_uuid uuid NULL;
  END IF;
END $$;
...
SELECT 'Columna client_uuid agregada a consents' as status 
WHERE EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'consents' AND column_name = 'client_uuid');
