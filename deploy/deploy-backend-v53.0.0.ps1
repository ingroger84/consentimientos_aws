...
Write-Host "🚀 DESPLIEGUE BACKEND v53.0.0" -ForegroundColor Cyan
Write-Host "Consolidación del Sistema de Perfiles" -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host ""
...
if (Get-Command tar -ErrorAction SilentlyContinue) {
    ...
    Write-Host "❌ tar no está disponible. Instala Git for Windows o WSL" -ForegroundColor Red
    exit 1
}
Write-Host ""
...
tar -xzf ~/backend-dist-v$VERSION.tar.gz
...
ssh -i $KEY_PATH "${SERVER_USER}@${SERVER_HOST}" $deployScript
...
ssh -i $KEY_PATH "${SERVER_USER}@${SERVER_HOST}" $migrationScript
...
ssh -i $KEY_PATH "${SERVER_USER}@${SERVER_HOST}" $restartScript
...
ssh -i $KEY_PATH "${SERVER_USER}@${SERVER_HOST}" $logsScript
Write-Host ""
